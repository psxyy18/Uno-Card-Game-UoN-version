...
while active:
    ...
    for inp in pygame.event.get():
        ...
        if inp.type == pygame.MOUSEBUTTONDOWN:
            ...
            if ((20 < m[0] < 55 or 210 < m[0] < 245) and 150 < m[1] < 180) and c.play_mode == pm.load: 
                if music_on:
                    sound.click.play()
                if c.easy:
                    c.easy = False
            ...
            if 0 < m[0] < 265 and 205 < m[1] < 270 and c.play_mode == pm.load:  
                if music_on:
                    sound.click.play()
                c.play_mode = pm.turn
            ...
            if c.player_playing:  
                ...
                if c.discard is False:
                    for i in range(625, 625 - 50 * len(c.player_list[0]), -50):  
                        if i < m[0] < i + 50 and 470 < m[1] < 585:
                            c.deck2.reverse()
    ...
    if c.play_mode == pm.load:
        ...
        text = pygame.font.Font(fnt.joe_fin, 20).render("PRINCE JOHN", True, (255, 238, 46))
        root.blit(text, [270, 4])
        text = pygame.font.Font(fnt.joe_fin, 20).render("MAID MARIAN", True, (255, 238, 46))
        root.blit(text, [830, 60])
        text = pygame.font.Font(fnt.joe_fin, 20).render("ROBINHOOD", True, (255, 238, 46))
        root.blit(text, [30, 410])
        text = pygame.font.Font(fnt.joe_fin, 20).render(c.message, True, (255, 238, 46))
        ...
        if c.player_playing: 
            ...
            if not c.drawn and not c.played: 
                ...
                elif c.current[0] == '+4' and c.special_check == 0:  # 
                    for _ in range(4):
                        try:
                            c.player_list[0].append(c.deck1.pop())
                        except:
            ...
            if c.play_lag == 140:  
                ...
                if win_dec and c.play_lag == 70:  
                    ...
                    if c.position != -1 and len(c.player_list[c.position]) == 1 and not c.uno[c.position]: 
